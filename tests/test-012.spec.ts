...
test('Change to Company 012', async ({ page }) => {
  ...
  await page.getByLabel('Current company', { exact: true }).fill('012');
  await page.getByLabel('Current company', { exact: true }).press('Enter');
  await expect(page.getByRole('heading', { name: 'PureChem Services,a Division' })).toHaveText('PureChem Services,a Division of Canadian Energy Services LP');
});
...
test('new vendor', async ({ page }) => {
  ...
  // Update test to different companies
  ...
  await expect(page.locator('#vendtablelistpage_1_Posting_VendGroup_input')).toContainText('VENDORS');

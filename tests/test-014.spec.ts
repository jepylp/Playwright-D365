...
test('Change to company 014', async ({ page }) => {
  ...
  await page.getByLabel('Current company', { exact: true }).fill('014');
  await page.getByLabel('Current company', { exact: true }).press('Enter');
  await expect(page.getByRole('heading', { name: 'Canadian Mud' })).toHaveText('Canadian Mud');
});
